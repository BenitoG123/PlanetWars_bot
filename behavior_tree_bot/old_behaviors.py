...
def if_neutral_planet_near(state):
    neutral_planets = [(planet,strength(state, planet)) for planet in state.neutral_planets()]
    neutral_planets = sorted(neutral_planets, key=lambda x: x[1])

    my_planets = [(planet,strength(state, planet)) for planet in state.my_planets()]
    my_planets = sorted(my_planets, key=lambda x: x[1], reverse=True)
    

    for neutral, neutral_power in neutral_planets:
        for mine, my_power in my_planets:
            distance = state.distance(neutral.ID, mine.ID)
            total_ships = neutral_power + (distance*neutral.growth_rate)
            if total_ships > 0:
                if distance < 7 and (total_ships < my_power-1):
...
def same_percent_attack(state, my_ships, enemy_planet, enemy_ships):
    ...
    required_percent = enemy_ships/my_ships + 0.5
    for planet in state.my_planets():
        logging.info(f"planet_ships: {planet.num_ships}")
        logging.info(f"sent ships: {math.floor(planet.num_ships * required_percent)}")
